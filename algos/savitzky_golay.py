...
def savitzky_golay(y, window_size, order, deriv=0):
    """
    Smooth data with Savitzky-Golay filter that preserves original data shape.
    
    A Savitzky-Golay filter smooths and optionally differentiate data.
    ...
    Args:
        y (array_like): shape (N,) the values of the time history of 
            the signal.
    ...
    Raises:
        ValueError: If window_size and order are not type int.
        TypeError: If winddow_size is not a positive odd number, or is
            too small (<order+2) for the polynomial order.
