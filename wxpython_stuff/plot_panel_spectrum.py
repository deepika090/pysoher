"""This module contains the PlotPanelSpectrum class that encapsulates a
matplotlib canvas within a wxpython widget. It is derived from the
PlotPanel class in plot_panel.py but has a specific interface for setting
and interacting with spectral data sets in the plot Axes. It can display
a plot of 1D spectral data in a variety of ways including: one spectrum,
a spectrum with an overlay, multiple lines in one spectrum, etc.

The PlotPanelSpectrum has a Figure and a Canvas and 'n' Axes. The user defines
...
Plotting Interface
-------------------
This version is more restrictive in that it requires a plot_option attribute
that describes the current state of the displayed data (e.g. whether x-axis
is shown, and if so is it Hz or PPM, etc.).  These plot_option settings can be
...
---------------
    ...
    PlotPanelSpectrum() - main call to instantiate a matplotlib canvas in a window
...
def _get_data_index(event, xvalue=None, reversex=False):
    """
...
class PlotPanelSpectrum(wx.Panel):
    """It is derived from the  PlotPanel class in plot_panel.py but has a
    specific interface for setting and interacting with spectral data sets in
    the plot Axes. It can display a plot of 1D spectral data in a variety of
    ways including: one spectrum, a spectrum with an overlay, multiple lines
    in one spectrum, etc.

    The PlotPanelSpectrum has a Figure and a Canvas and 'n' Axes. The user
    defines the number of axes on Init and this number cannot be changed
    thereafter. However, the user can select which of the n axes are actually
    ...
    Example (non-functional snippet - see example code at end of module):

        lines = self.make_data(lines=3, points=2048)
        bob = lines.copy()
        dbob = {'data':bob,'line_color_real':'green'}
        # There are three Axes, here we pass in a list, a dict, and a list
        # Where values are not specified (eg. the lists) default values are
        # used. If values are passed in a dict, those become the new defaults.
        data = [[lines],[dbob],[lines]]
        # see 'Setting Data into the Object' in module docstring
    ...
    Args:
        ...
        data (list of lists): default=None, A default numpy array of 1024 zeros is
            is created for each of the N Axes in the PlotPanelSpectrum object. These
            are put into a list of lists and set_data(data) is called. If the user
            passes in a list of lists of numpy arrays (or properly formatted dicts
            with data and prefs attributes) then set_data() is called on that.

        prefs (object): default=None, An object with attributes that control the
            display of data within the PlotPanelSpectrum. These values can be updated
            to reflect external changes via the 'set_xxx()' methods. The following
        ...
        dataset (object): default=None, If None, a default object is created.
            This object has to contain a minimum set of attributes to allow the
            PlotPanelSpectrum to calculate certain transforms like Hz to PPM
            values on the x-axis, etc. The following attributes are required:
                self.sw = 2500
                self.frequency = 123.9
                self.resppm = 4.7
                self.phase_1_pivot = 2.01

        line_width (float): default=1.0, Deprecated, use prefs.line_width
            attribute instead.
    ...
    def _default_data(self):
        """ utility function for default creation """
        data = []
    ...
    def get_values(self, event, reversex=False):
        ...
        value = []
        x0, y0, x1, y1 = event.inaxes.dataLim.bounds
        indx = _get_data_index(event, reversex=reversex)
...
class CursorSpan:
    ...
    def release(self, event):
        ...
        if event.xdata == self.pressv:
            ...
            if self.do_refs_select_event:
                ...
                if self.do_refs_select_event and data_test:
                    vmin = self.pressv
                    rindex = _get_data_index(event, xvalue=vmin, reversex=self.parent.reversex)
                else:
                    rindex = None
        ...
        if self.do_refs_select_event and data_test:
            ...
            imin = _get_data_index(event, xvalue=vmin, reversex=self.parent.reversex)
            imax = _get_data_index(event, xvalue=vmax, reversex=self.parent.reversex)
...
class MyFrame(wx.Frame):
    ...
    def _create_fake_prefs(self):
        self.prefs = _fake_prefs()
    

#------------------------------------------------------------------------------

class _fake_prefs(object):
